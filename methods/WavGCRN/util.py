...
def graph_loss(A, decoder_input):
    row_sum = torch.sum(A, 1)
    D = torch.diag_embed(row_sum)
    L = D - A
    # print("feature shape: ", decoder_input.shape)
    n = decoder_input.shape[2]
    out_dim = decoder_input.shape[1]
    batch = decoder_input.shape[0]
    loss_gl1 = 0
    for i in range(batch):
        X = decoder_input[i, :, :]
        loss_gl1 += torch.trace(X.T @ L[i, :, :] @ X) / n ** 2
    loss_gl1 /= batch
    loss_gl2 = -0.3 * torch.sum(torch.log(row_sum)) / n + 0.7 * torch.norm(A, p='fro') / n ** 2
    return loss_gl2 # + loss_gl1

def hy_mae(preds, labels, A1, A2, decoder_input1, decoder_input2, null_val=np.nan): #A.shape=(64,207,207)
    loss_mae = masked_mae(preds, labels, null_val)
    #loss_gl = graph_loss(A1, decoder_input1)
    #loss_gl += graph_loss(A2, decoder_input2)
    #total_loss = loss_mae + loss_gl
    return loss_mae #total_loss
