...
import time
from util import *
from trainerp import Trainer
import yaml
from netp import DGCRN
import setproctitle
import os
...
parser.add_argument('--step_size1', type=int, default=2500, help='step_size')#2500
...
def get_config(config_path="config.yml"):
    ...
    return config
config = get_config(config_path="/lus/theta-fs0/projects/hpcbdsm/qqian/DGWRN/methods/DGCRN/idgl.yml")
...
def main(runid):
    ...
    print(args)
    adj1 = predefined_A[0].unsqueeze(0).repeat(64, 1, 1)
    adj2 = predefined_A[0].unsqueeze(0).repeat(64, 1, 1)

    nParams = sum([p.nelement() for p in model.parameters()])
    print('Number of model parameters is', nParams)

    engine = Trainer(adj1, adj2, model, args.learning_rate, args.weight_decay, args.clip,
    ...
    if args.TEST_ONLY:
        ...
        for iter, (x,
            ...
            with torch.no_grad():
                engine.model.eval()
                preds, _, _, _, _ = engine.model(testx, md1=adj1, md2=adj2, ycl=testy)
        ...
        for i in range(1, args.epochs + 1):
            ...
            #adj1, adj2 = engine.graph_learning(trainx, trainy[:, 0, :, :], trainycl, idx=None, batches_seen=batches_seen)
            #print("gl", gl, i)
            if i >= gl + 3 and i <= args.epochs - 200:
                #print("get in if ")
                if (sorted(his_loss)[1] - sorted(his_loss)[0] < 0.001 and sorted(his_loss)[0] != valid_loss[-1] and valid_loss[-1] > valid_loss[-2] and sorted(his_loss)[0] <= 2.9) or i >= gl + 200:
                    print("best of this graph: ", sorted(his_loss)[0], sorted(his_loss)[1])
                    best_graph.append(sorted(his_loss)[0])
                    #print("best of each graph: ", best_graph)
                    his_loss = []
                    print("strat graph tuning after: ", i - gl)
                    gl = i
                    oldadj1 = adj1
                    oldadj2 = adj2                    
                    adj1, adj2 = engine.graph_learning(trainx, trainy[:, 0, :, :], trainycl, idx=None, batches_seen=batches_seen)
                    ...
                    engine = Trainer(adj1, adj2, model, args.learning_rate, args.weight_decay, args.clip, args.step_size1, args.seq_out_len, scaler, device, args.cl, args.new_training_method)
        ...
        print("best results of each graph: ", best_graph)
